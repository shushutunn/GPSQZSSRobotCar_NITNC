...
void setup() {
  ...
  Serial.println(tmpp,10);
  while (digitalRead(BTN_1) == HIGH);
 
    limittime=(gps.time.hour()+9)*3600+(gps.time.minute()+3)*60+gps.time.second();
...
//     Serial.print("if=");Serial.println(limittime-int((gps.time.hour()+9)*3600+(gps.time.minute())*60+gps.time.second())); 
